...
//    private static final int DB_BATCH_SIZE = 10;       // Lote de inserção no banco
    ...
    public static void main(String[] args) {
        ...
        try {
            // Caminho do arquivo local comentado
//            String localFilePath = "C:\\Users\\vit_o\\FlyOn\\projeto-etl-xlsx\\src\\main\\resources\\VRA_2022_01.xlsx";
            ...
            System.out.println("[ETL] Iniciando acesso remoto ao S3...");

            // Obtém InputStream diretamente do S3
            InputStream s3InputStream = S3Downloader.getFileStream("s3-bucket-name", s3FilePath);

            // Carrega o workbook com o InputStream
            XlsxExtractor.loadWorkbook(s3InputStream);

            System.out.println("[ETL] Acesso remoto ao S3 concluído.");

            try {
                // Versão sem o FilePath
                int totalRows = XlsxExtractor.returnTotalRows();
...
//                List<Integer> batchAssentos = new ArrayList<>(DB_BATCH_SIZE);
                ...
                for (int startRow = 1; startRow <= rowsToProcess; startRow += EXTRACT_BATCH_SIZE) {
                    ...
                    List<List<String>> batchRawData = XlsxExtractor.extractBatchData(startRow, currentBatchSize);
                    ...
                    for (List<String> rawData : batchRawData) {
                        ...
                        // Armazena nos lotes (comentado por enquanto)
...
//                        }
                ...
                // Envia lotes restantes (comentado)
